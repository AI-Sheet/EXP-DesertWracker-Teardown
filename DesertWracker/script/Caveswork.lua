...
local NUM_CUBES = 40              -- Total cubes to spawn (adjustable)
...
function CarveInteriorFeatures(cube)
    ...
    -- Для входного куба: всегда используем глубокий проход с верхней грани.
    ...
    -- Для остальных кубов: исключаем возможность использования "winding_passage".
...
function CarveWindingPassage(cube, center, isEntrance)
    ...
    if isEntrance then
        -- Логика для входного куба остаётся прежней.
        ...
        if entranceStyle == "crevice" then
            ...
            -- Дополнительные боковые вырезы для естественности.
            ...
            local endPoint = { center[1], math.max(0, sy - depth), center[3] }
    ...
    -- Для обычных кубов: форсируем создание прохода, начинающегося на верхней грани.
    local passageSize = math.random(MEDIUM_BRUSH_MIN, MEDIUM_BRUSH_MAX)
    local startPoint = { center[1], sy, center[3] }           -- Верхняя грань (центр сверху)
    local endPoint = { center[1], sy - passageSize, center[3] }  -- Немного ниже верхней грани

    -- Генерация прямолинейного пути через заданное число сегментов.
    ...
    -- Опционально: в нижней части прохода можно добавить небольшую камеру.
    if math.random() < 0.7 then
